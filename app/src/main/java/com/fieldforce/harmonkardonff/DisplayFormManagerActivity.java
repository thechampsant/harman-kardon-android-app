...
public class DisplayFormManagerActivity extends InnosolsActivity {
	...
	OnClickListener img = new OnClickListener() {
		...
		public void onClick(View v) {
			// TODO Auto-generated method stub
			Intent I = new Intent(DisplayFormManagerActivity.this,
	...
	private boolean Validate(MDisplay model) {
		...
		if (Integer.valueOf(model.Price) == 0) {
			ShowToast("Please Enter a Valid Display Price");
			return false;
		} else if(Integer.valueOf(model.Price)<=0){
			ShowToast("Display Price can not be negative");
			return false;
		} else
			return true;
	...
	private void UpdateDisplayModel() throws IOException {
		...
		if (IsSucc) {
			...
			ShowToast("Please Enter a Valid Model Price");
		}
	...
	private void DO_GPRS_Communication() {
		...
		try {
			 ...
			 * }
			 */
			MDisplay mdispaly = updateDisplayModel();
			if (Validate(mdispaly));
//				result = server.CreateNewDP(mdispaly);
		} catch (Exception e) {
