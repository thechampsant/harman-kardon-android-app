...
public class QuizActivity extends InnosolsActivity {

	private VM_Quiz model = new VM_Quiz();
	...
	protected void onCreate(Bundle savedInstanceState) {
		try {
			...
			initilizeControls();
			attachClickable();
			LoadQuestionFromLocal((Quiz) IntentFactory.getData(Keys.QUIZ));
			// startTimer();
	...
	private void startTimer() {
		if (model.IsQuizValid()) {
			...
			ArrayList<QuizIDTimeDetailModel> arrQuizIDModel = MainActivity.Current.db
					.FetchAllData(new QuizIDTimeDetailModel()).where(
							"UserName", MainActivity.MyInfo.EmployeeCode);

			if (arrQuizIDModel != null && arrQuizIDModel.size() > 0) {
				for (QuizIDTimeDetailModel quizIDModel : arrQuizIDModel) {
			...
			public void run() {
				runOnUiThread(new Runnable() {
					...
					public void run() {
						...
						seconds -= 1;
						ArrayList<QuizIDTimeDetailModel> arrQuizIDModel = MainActivity.Current.db
								.FetchAllData(new QuizIDTimeDetailModel())
								.where("UserName",
						...
						QuizIDTimeDetailModel quizIDTime = new QuizIDTimeDetailModel();
						...
						if (minutes == -1) {
							timer.cancel();
							QuizActivity.this.goToSubmitScreen();
							submitQuiz();
						}
						// Log.e("key","value");
					}
	...
	private void LoadQuestionFromLocal(final Quiz quiz) {
	...
	private void attachClickable() {
		...
		getView(R.id.lyt_back).setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				// TODO Auto-generated method stub
				QuizActivity.this.goToBackQuestion();
			}
		});
		...
		getView(R.id.lyt_submit).setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				// TODO Auto-generated method stub
				QuizActivity.this.confirmSubmitQuiz();
			}
		});
	...
	private void setQuestionText() {
		// TODO Auto-generated method stub
		if (model.IsQuizValid() && !model.isNextDisabled()) {
			this.SetTextViewAsString(R.id.txt_question,
					model.currentQuiz.Question);
		}

	}

	private void setQuestionStatusVisibility() {
		if (model.IsAnswered() || model.isQuizCompleted())
	...
	private void setWarningForQuiz() {
		setQuizQuestionStatusList();
	...
	private void setQuizQuestionStatusList() {
		if (model.isQuizCompleted()) {
			...
			GenricAdapter adaptor = new GenricAdapter(this,
					R.layout.listitem_quiz_status).setData(model.QuizQuestions);
			adaptor.setGenricAdapter(new IAdapter() {
				...
				public void setItemView(Object obj, View convertView, int index) {
					// TODO Auto-generated method stub
					Quiz info = (Quiz) obj;
					int QuestionNo = model.QuizQuestions.indexOf(info) + 1;
	...
	void setQuizFinalScreenVisibility() {
		if (model.isQuizCompleted()) {
	...
	private RadioGroup createRadioButton(ArrayList<Quiz> _data, boolean isdone) {
	...
	private void createCheckBoxes(ArrayList<Quiz> _data, boolean isdone) {
		rgm.removeAllViews();
	...
	private void goToPendingQuestion(int index) {
		...
		model.setCurrentQuiz();
		setQuizStatusheader();
		setQuizQuestionSerial();
		onClick(true);
	}
	...
	private void setFirstQuestion() {
		if (model.isQuizAttemptedBefore()) {
			model.setCurrentQuiz();
	...
	private void submitQuiz() {
		showProgress("saving quiz ...");

		model.submitQuizOnOnline(new IEvent() {
			...
			public void Completed(Object Sender, Object Args) {
				...
				hideProgress();
				if (model.IsSuccess) {
					model.currentQuiz.AnsweredOn = getMonthYear(getCurrentDate());
					model.currentQuiz.InsertOrUpdate();
...
//					showQuizStatus();
					...
					new Dialog(QuizActivity.this).show(model.Message);
			}
		});
	...
	private void showQuizStatus() {
		new AlertDialog.Builder(this).setTitle("Quiz Result!")
				.setMessage(model.getQuizResultMessage()).setCancelable(false)
	...
	private void onClick(boolean isdone) {
		...
		if (!model.isQuizCompleted()) {
			...
			if (model.IsQuizValid()) {
				model.currentQuiz.IsQuizCompleted = "true";
				model.currentQuiz.InsertOrUpdate();
			}
			model.setIndexToFinal();
		}
